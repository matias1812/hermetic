...
impl DHRatchet {
    ...
    pub fn dh_ratchet(&mut self, remote_public: PublicKey) {
        ...
        hkdf.expand(b"receiving_chain", &mut self.state.receiving_chain_key).unwrap();
        hkdf.expand(b"header_key_recv", &mut self.state.header_key_recv).unwrap();
        ...
        hkdf2.expand(b"sending_chain", &mut self.state.sending_chain_key).unwrap();
        
        if let Some(nhk_s) = self.state.next_header_key_send {
            self.state.header_key_send = nhk_s;
        } else {
            hkdf2.expand(b"header_key_send", &mut self.state.header_key_send).unwrap();
        }
        
        let mut next_nhk_s = [0u8; HEADER_KEY_SIZE];
        hkdf2.expand(b"next_header_key_send", &mut next_nhk_s).unwrap();
    ...
    fn advance_sending_chain(&mut self) -> [u8; MESSAGE_KEY_SIZE] {
        ...
        hkdf.expand(b"chain_key", &mut self.state.sending_chain_key).unwrap();
    ...
    fn encrypt_header(&self, header: &Header) -> Vec<u8> {
        ...
        let ciphertext = cipher.encrypt(&nonce.into(), header_bytes.as_ref()).unwrap();
    ...
    fn take_skipped_key(&mut self, message_number: u32) -> Option<[u8; MESSAGE_KEY_SIZE]> {
        if let Some(idx) = self.state.skipped_keys.iter().position(|sk| sk.message_number == message_number) {
    ...
    fn decrypt_body(&self, ciphertext: &[u8], nonce: &[u8], key: &[u8; 32], aad: &[u8]) -> Result<Vec<u8>, RatchetError> {
        use chacha20poly1305::{XChaCha20Poly1305, KeyInit, aead::{Aead, Payload}};
        ...
        cipher.decrypt(&full_nonce.into(), payload)
