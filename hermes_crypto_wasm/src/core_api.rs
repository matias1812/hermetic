...
use crate::ratchet::x3dh::{PreKeyBundle, InitialHandshake, generate_prekey_bundle, initiator_x3dh, responder_x3dh};
...
impl HermesCore {
    ...
    pub fn close_session(&mut self) {
        ...
        if let Some(mut key) = self.mldsa_secret_seed.take() { // ML-DSA seed zeroized
            key.zeroize();
        }
    ...
    pub fn generate_prekey_bundle(&mut self, opk_id_opt: Option<String>) -> Result<String, JsValue> {
        ...
        let (bundle, mut spk_sec, opk_sec_opt) = generate_prekey_bundle(&ik_sec, &sign_sec, &mldsa_sec, opk_id_opt.clone(), &pqc_pk_bytes);
    ...
    pub fn create_session_from_bundle(&mut self, contact_id: &str, bundle_json: &str) -> Result<String, JsValue> {
        ...
        let (mut sk, handshake) = initiator_x3dh(&ik_sec, &bundle, &mut pqc_ciphertext_out, &mut pqc_shared_secret_out)
    ...
    pub fn accept_session_handshake(&mut self, contact_id: &str, handshake_json: &str) -> bool {
        ...
        let opk_sec = handshake.one_time_pre_key_id.as_ref().and_then(|id| self.opk_secrets.remove(id));
        ...
        let mut sk = match responder_x3dh(&ik_sec, &spk_sec, opk_sec.as_ref(), &handshake, &pqc_shared_secret) {
            Ok(key) => key,
            Err(_) => return false,
        };
    ...
    pub fn create_session(
        ...
        let ratchet = DHRatchet::new_with_role(&shared_secret, remote_public, local_sk, local_public, is_alice);
        self.sessions.insert(contact_id.to_string(), ratchet);
        true
    }

    /// Cifra un mensaje 1:1
    pub fn encrypt_message(&mut self, contact_id: &str, plaintext: &str) -> Result<Vec<u8>, String> {
        let ratchet = self.sessions.get_mut(contact_id)
            .ok_or_else(|| "Fail-Closed: Session not found".to_string())?;
            
        let encrypted = ratchet.encrypt(plaintext.as_bytes(), b"");
        serde_json::to_vec(&encrypted).map_err(|e| format!("Fail-Closed: Serialization failed: {}", e))
    }

    /// Descifra un mensaje 1:1
    pub fn decrypt_message(&mut self, contact_id: &str, ciphertext_json: &[u8]) -> Result<String, String> {
        let ratchet = self.sessions.get_mut(contact_id)
        ...
        let plaintext_bytes = ratchet.decrypt(&msg, b"")
            .map_err(|e| format!("Decryption failed: {:?}", e))?;
            
        String::from_utf8(plaintext_bytes).map_err(|_| "Invalid UTF-8 in decrypted message".to_string())
    }
    ...
    pub fn create_group(&mut self, _group_id: &str, _member_ids: js_sys::Array) -> Result<Vec<u8>, String> {
        Err("NotImplemented: Group creation pending strict GroupManager FFI consolidation".to_string())
    }

    /// Rotar llave de grupo (FAIL-CLOSED)
    pub fn rotate_group_key(&mut self, _group_id: &str) -> Result<Vec<u8>, String> {
        Err("NotImplemented: Group key rotation pending strict GroupManager FFI consolidation".to_string())
    }

    /// Cifrar mensaje de grupo (FAIL-CLOSED: prohíbe simulación de cifrado devolviendo texto en claro)
    pub fn encrypt_group_message(&self, _group_id: &str, _plaintext: &str) -> Result<Vec<u8>, String> {
        Err("NotImplemented: Group message encryption pending strict FFI core consolidation".to_string())
    }

    /// Descifrar mensaje de grupo (FAIL-CLOSED)
    pub fn decrypt_group_message(&self, _group_id: &str, _ciphertext: &[u8]) -> Result<String, String> {
        Err("NotImplemented: Group message decryption pending strict FFI core consolidation".to_string())
    }
    ...
    pub fn generate_mnemonic(&self) -> Result<String, String> {
        const WORDLIST: [&str; 256] = [
            ...
            "blade", "blame", "blank", "blast", "bleak", "blind", "blood", "blossom",
    ...
    pub fn derive_recovery_key(&self, mnemonic: &str) -> Result<Vec<u8>, String> {
        ...
        let mut okm = [0u8; 32];
        hk.expand(b"hermes-backup-key-v1", &mut okm).map_err(|_| "Fallo en derivación HKDF".to_string())?;
        Ok(okm.to_vec())
    }
    ...
    pub fn decrypt_with_recovery_key(&self, mnemonic: &str, data: &[u8]) -> Result<Vec<u8>, String> {
        let key = self.derive_recovery_key(mnemonic)?;
        if data.len() < 24 {
    ...
    pub fn compute_admin_sig(&self, challenge: &str, sk_hex: &str) -> Result<String, String> {
        ...
        let sk_bytes = hex::decode(sk_hex).map_err(|_| "Formato hexadecimal inválido".to_string())?;
        let sk = SigningKey::from_bytes(sk_bytes.as_slice().try_into().map_err(|_| "Longitud de llave Ed25519 inválida".to_string())?);
