...
impl HermesCore {
    ...
    pub fn derive_recovery_key(&self, mnemonic: &str, user_id_hash: &str) -> Result<Vec<u8>, String> {
        use hkdf::Hkdf;
        use sha2::Sha256;
    ...
    pub fn derive_recovery_proof(&self, mnemonic: &str, user_id_hash: &str) -> Result<Vec<u8>, String> {
        use hkdf::Hkdf;
        use sha2::Sha256;
        let salt = format!("hermes-recovery-salt:{}", user_id_hash);
        let hk = Hkdf::<Sha256>::new(Some(salt.as_bytes()), mnemonic.as_bytes());
    ...
    pub fn decrypt_group_ephemeral_image(
        ...
        if key_bytes.len() != 32 {
            key_bytes.zeroize();
            return Err(JsValue::from_str("Fail-Closed: la clave debe ser de 32 bytes"));
        }
        let nonce_bytes = hex::decode(nonce_hex)
            .map_err(|_| JsValue::from_str("Fail-Closed: nonce_hex no es hexadecimal válido"))?;
        if nonce_bytes.len() != 12 {
            key_bytes.zeroize();
            return Err(JsValue::from_str("Fail-Closed: el nonce debe ser de 12 bytes"));
        }
        let ciphertext_bytes = hex::decode(ciphertext_hex)
            .map_err(|_| JsValue::from_str("Fail-Closed: ciphertext_hex no es hexadecimal válido"))?;
