...
fn test_invariants_no_key_reuse() {
    ...
    for i in 0..10 {
        ...
        assert!(!used_ciphertexts.contains(&enc.ciphertext), "INVARIANTE ROTO: Reutilización de ciphertext detectada");
        assert!(!used_nonces.contains(&enc.nonce), "INVARIANTE ROTO: Reutilización de nonce detectada");
        
        used_ciphertexts.push(enc.ciphertext.clone());
        used_nonces.push(enc.nonce);

        let dec = bob.decrypt(&enc, b"aad").expect("Descifrado en ráfaga debe tener éxito");
...
fn test_network_simulation_packet_reordering_and_loss() {
    ...
    let dec2 = bob.decrypt(&m2, b"").expect("Descifrado de m2 (salto de m0 y m1) debe tener éxito");
    assert_eq!(dec2, b"msg 2");

    // Ahora llegan m0 y m1 fuera de orden desde skipped_keys
    let dec0 = bob.decrypt(&m0, b"").expect("Descifrado de m0 (skipped key) debe tener éxito");
    assert_eq!(dec0, b"msg 0");

    let dec1 = bob.decrypt(&m1, b"").expect("Descifrado de m1 (skipped key) debe tener éxito");
    assert_eq!(dec1, b"msg 1");

    // Simulación de pérdida permanente de m3: llega m4 directamente
    let dec4 = bob.decrypt(&m4, b"").expect("Descifrado de m4 tras pérdida de m3 debe tener éxito");
...
fn test_fuzzing_boundary_corrupted_messages() {
    ...
    assert!(bob.decrypt(&enc, b"auth").is_err(), "Módulo E: El sistema debe rechazar ciphertext corrupto sin pánico");

    // Corrupción en AAD
    enc.ciphertext[0] ^= 0xFF; // restaurar
    assert!(bob.decrypt(&enc, b"wrong_auth").is_err(), "Módulo E: El sistema debe rechazar AAD inválido sin pánico");
}
