...
fn test_pqc_corruption_changes_root_key() {
    ...
    assert_ne!(root_key_normal, root_key_corrupted, "LA LLAVE MAESTRA DEBE SER DISTINTA ANTE CORRUPCIÓN PQC");
}

#[wasm_bindgen_test]
fn test_wasm_identity_and_signatures() {
    let core = hermes_crypto_wasm::core_api::HermesCore::new();
    let keys_json = core.generate_identity_keys().expect("Debe generar llaves de identidad");
    ...
    let sig = core.compute_admin_sig(challenge, sk_hex).expect("Debe firmar el desafío");
    assert!(!sig.is_empty());
}
