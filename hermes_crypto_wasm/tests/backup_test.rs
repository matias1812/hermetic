...
fn test_encrypt_decrypt_backup_roundtrip() {
    ...
    assert!(result_before_unlock.is_err(), "Debe fallar si la bóveda está bloqueada");
    ...
    let decrypted = core.decrypt_backup(&ciphertext, None).expect("Falló el descifrado");
    assert_eq!(decrypted, plaintext, "El texto descifrado debe coincidir con el original");

    // Descifrar con password explícito (simulando restauración externa)
    let decrypted_pwd = core.decrypt_backup(&ciphertext, Some("password_dummy".to_string())).expect("Falló el descifrado con pwd");
...
fn test_decrypt_backup_tamper_detection() {
    ...
    assert!(decrypt_result.is_err(), "Debe fallar si los datos fueron alterados");
}
...
fn test_encrypt_backup_random_nonce() {
    ...
    assert_eq!(dec1, dec2, "Ambos ciphertexts deben descifrar al mismo plaintext");
}
...
fn test_decrypt_backup_wrong_password() {
    ...
    assert!(decrypt_result.is_err(), "Debe fallar si se restaura con password incorrecto");
}
