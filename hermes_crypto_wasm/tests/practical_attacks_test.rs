...
fn test_practical_attacks_replay_and_reflection() {
    ...
    let dec1 = bob
        .decrypt(&msg1, b"aad")
        .expect("Descifrado inicial legítimo");
    assert_eq!(dec1, b"secret message");

    // 2. Ataque de repetición (Replay attack): el atacante reenvía msg1 capturado a Bob
    assert!(
        bob.decrypt(&msg1, b"aad").is_err(),
        "Módulo I: Ataque de repetición (Replay) debe ser rechazado explícitamente"
    );

    // 3. Ataque de reflexión (Reflection attack): el atacante refleja el mensaje de Alice hacia Alice misma
    assert!(
        alice.decrypt(&msg1, b"aad").is_err(),
        "Módulo I: Ataque de reflexión hacia el emisor debe ser rechazado"
    );
}
...
fn test_practical_attacks_skipped_key_exhaustion_dos_protection() {
    ...
    let dec = bob
        .decrypt(&malicious_msg, b"")
        .expect("Descifrado normal debe funcionar");
